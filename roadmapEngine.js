...
const URGENCY = {
  id:         { base: 100, label: 'ID & Documents',    icon: '📋', color: '#2d6a4f' },
  housing:    { base: 90,  label: 'Housing',           icon: '🏠', color: '#1a3d2b' },
  food:       { base: 85,  label: 'Food & Benefits',   icon: '🍽️', color: '#e07b2a' },
  employment: { base: 70,  label: 'Employment',        icon: '💼', color: '#52b788' },
  mentalHealth:{ base: 65, label: 'Mental Health',     icon: '💬', color: '#6b6b63' },
  family:     { base: 55,  label: 'Family Reconnection',icon: '👨‍👩‍👧', color: '#9e9e94' },
  education:  { base: 45,  label: 'Education',         icon: '📚', color: '#52b788' },
  community:  { base: 40,  label: 'Community',         icon: '🤝', color: '#2d6a4f' },
};

// Steps per need, keyed by borough context
const STEPS = {
  id: {
    Bronx:     [
  ...
  community: {
    default: [
      { id:'c-1', text:'Connect with Revia\'s peer network', detail:'Other people who\'ve been through this. Same borough, same experience. No judgment.', time:'Right now' },
      { id:'c-2', text:'Find a mentor', detail:'Community leaders and formerly incarcerated mentors matched to your background. Free through Revia.', time:'This week' },
    ],
  },
};

// Auto-added needs based on time away
function getAutoNeeds(timeAway) {
  const extra = [];
  if (timeAway === '5-15 years' || timeAway === '15+ years') {
    extra.push('mentalHealth');
  }
  if (timeAway === '15+ years') {
    extra.push('community'); // they've been gone so long, community is critical
  }
  return extra;
}
...
export function generateRoadmap({ timeAway, borough, needs }) {
  ...
  const allNeeds = [...new Set(['id', ...needs, ...autoNeeds])]; // id always first

  const sections = allNeeds.map(needKey => {
    ...
    // Score: base + user-selected bonus + time-away bonus
    let score = meta.base;
    if (needs.includes(needKey)) score += 20;
    if (needKey === 'id') score = 999; // always top
    if ((needKey === 'mentalHealth') && timeAway === '15+ years') score += 15;

    return { key: needKey, score, steps, ...meta, isAuto: autoNeeds.includes(needKey) && !needs.includes(needKey) };
  }).filter(Boolean);
